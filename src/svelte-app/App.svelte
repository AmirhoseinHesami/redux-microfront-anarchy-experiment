...
<div class="h-full flex flex-col justify-between">
    ...
    <button
      class="w-full mt-6 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white py-3 px-4 rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2 relative overflow-hidden cursor-pointer"
      on:click={handleClear}
      disabled={$basket.length === 0 || isLoading}
    >
      <div class="absolute inset-0 bg-black/10 transition-transform duration-300" 
        class:translate-x-0={isLoading}
        class:-translate-x-full={!isLoading}
      ></div>
